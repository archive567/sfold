...
data SBar = SBar
  { sbarValue :: Double
  , sbarFirstTime :: Maybe Double
  , sbarLastTime :: Maybe Double
  , sbarResult :: [(Double, Double)]
  } deriving (Show)
...
predFold budSBar = SFold to ma state' sweep flush
  where
    ...
    ma s0@(SBar v0 ft0 lt0 out0) s1@(SBar v1 ft1 lt1 out1) = bar
      where
        v = v0 + v1
        ft = orMaybe identity min ft0 ft1
        lt = orMaybe identity max lt0 lt1
        out = out0 <> out1
        bar =
          if budSBar s0 s1
